...
expect(stringField(compileArtifact, "schema") === "octra-vitals-program-compile-v0", "compile artifact schema changed");
expect(stringField(compileArtifact, "source_path") === "program/main.aml", "compile artifact source_path must point at program/main.aml");
expect(stringField(compileArtifact, "source_hash") === sourceHash, "compile source_hash does not match program/main.aml");
...
expect(prefixedHash(stringField(certificateArtifact, "source_hash")) === sourceHash, "certificate source_hash does not match program/main.aml");
...
for (const functionName of requiredFunctions) {
  expect(abiFunctionNames.has(functionName), `ABI missing ${functionName}`);
}
for (const functionName of forbiddenFunctions) {
  expect(!abiFunctionNames.has(functionName), `ABI still exposes removed function ${functionName}`);
}
...
for (const snippet of forbiddenSourceSnippets) {
  expect(!source.includes(snippet), `AML source still contains forbidden storage/path snippet ${snippet}`);
}
