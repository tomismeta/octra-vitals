...
`VITALS_LAB_HISTORY_WRITE_OU` is the Vitals-owned budget for owner-signed SQL writes. The Lab worker passes it to `octra-sqlite` as both `--ou` and `OCTRA_SQLITE_WRITE_OU`; if it is unset, Vitals falls back to `OCTRA_SQLITE_WRITE_OU`, then `OCTRA_SQLITE_VERIFY_WRITE_OU`, and otherwise refuses to write rather than using octra-sqlite's low default. Use this for devnet/stage fee headroom without changing AML deploy or program-update budgets. The setup script pins octra-sqlite `0.6.3+` because earlier builds do not honor this write-OU surface.
...
Lab reads do not require a token. The query endpoint accepts bounded read-only `select` / `with` SQL so reviewers can inspect the derived mirror without wallet or operator access, and the gateway applies global, per-client, concurrency, origin, content-type, body-size, and body-time guards before spawning `octra-sqlite`. Each query response includes query provenance with the database Circle, RPC URL, JSON-RPC method, Circle method, normalized SQL, limit, and normalized SQL hash. Vitals does not expose raw JSON-RPC request/response traces from lab queries. Automated repair/backfill runs in the separate mirror worker. The legacy HTTP admin route is disabled by default (`VITALS_LAB_ADMIN_HTTP_ENABLED=0`), is loopback-only when explicitly enabled, and still requires `VITALS_LAB_HISTORY_WRITE_TOKEN`. Keep that token in `lab-history.env`, not `gateway.env`, git, or chat.
...
POST /api/lab/mirror/sync  # disabled by default; loopback JSON + token when enabled
```

`/api/lab/query` accepts same-origin `application/json` requests containing bounded read-only SQL:
...
`/api/lab/mirror/sync` reads canonical AML history through the direct verified AML path, then writes derived rows into the lab database. This endpoint is an operator repair/backfill tool; the primary automated path is the separate Lab mirror worker. The response includes `row_count`, `pending_row_count`, and `complete` so operators can tell whether more sync passes are needed.
