...
const outPath = process.env.VITALS_PROGRAM_V1_OVERFLOW_REHEARSAL_REPORT ||
  join(root, "reports", `program-v1-overflow-rehearsal-${new Date().toISOString().replace(/\.\d{3}Z$/, "Z").replace(/[:]/g, "")}.json`);
const rowCount = Number(process.env.VITALS_PROGRAM_V1_OVERFLOW_ROWS || HISTORY_V1_CAPSULE_ROW_LIMIT + 2);
...
function assertDevnetRpc(url: string): void {
  if (!/devnet/i.test(url)) throw new Error(`refusing overflow rehearsal against non-devnet RPC URL: ${url}`);
}
...
function observedAtForOffset(offset: number): string {
  return new Date(Date.UTC(2026, 5, 23, 0, 0, offset)).toISOString().replace(/\.\d{3}Z$/, "Z");
}
...
async function writeReport(report: unknown): Promise<void> {
  ...
  console.log(stableJson({
    ...
    row_count: (report as any).row_count,
...
if (!Number.isInteger(rowCount) || rowCount <= HISTORY_V1_CAPSULE_ROW_LIMIT || rowCount > HISTORY_V1_CAPSULE_ROW_LIMIT * 2) {
  throw new Error(`VITALS_PROGRAM_V1_OVERFLOW_ROWS must be ${HISTORY_V1_CAPSULE_ROW_LIMIT + 1}..${HISTORY_V1_CAPSULE_ROW_LIMIT * 2}`);
}
...
const baseSnapshotPath = process.env.VITALS_PROGRAM_V1_OVERFLOW_BASE_SNAPSHOT || join(root, "build", "latest_snapshot.json");
...
const submissions: Array<{ snapshot_id: string; tx_hash: string | null; readback: unknown }> = [];
let lastCall: RecordSnapshotCallV1 | null = null;

for (let index = 1; index <= rowCount; index += 1) {
  const snapshot = snapshotAt(base, observedAtForOffset(index), baseEpoch + index);
  const call = await buildRecordSnapshotCall(snapshot, { recordVersion: "v1", snapshotIndex: index, submitEnabled: true });
  if (call.commit_mode !== "v1") throw new Error("overflow rehearsal expected v1 calls");
  lastCall = call as RecordSnapshotCallV1;
  const submit = await submitSnapshotCall(call, {
    submitEnabled: true,
  ...
  submissions.push({
    ...
    tx_hash: submit.tx_hash || null,
    readback: submit.readback
  });
}

if (!lastCall) throw new Error("overflow rehearsal did not submit any rows");
...
const [
  ...
  circleProgramViewAtUrl<string>(rpcUrl, circleId, "get_open_capsule_base_id"),
  circleProgramViewAtUrl<number>(rpcUrl, circleId, "get_open_capsule_segment"),
...
const overflowRows = rowCount - HISTORY_V1_CAPSULE_ROW_LIMIT;
const checks = {
  snapshot_count_matches: Number(snapshotCount) === rowCount,
  capsule_count_is_one: Number(capsuleCount) === 1,
  latest_capsule_id_is_first_segment: latestCapsuleId === "2026-06-23T00.0000",
  sealed_row_count_is_limit: meta.row_count === String(HISTORY_V1_CAPSULE_ROW_LIMIT).padStart(6, "0"),
  sealed_body_is_limit_rows: latestCapsuleBody.length === HISTORY_V1_CAPSULE_ROW_LIMIT * HISTORY_V1_ROW_LEN,
  ...
  open_capsule_base_id_is_same_half: openCapsuleBaseId === "2026-06-23T00",
  open_capsule_segment_is_one: Number(openCapsuleSegment) === 1,
  open_capsule_id_is_second_segment: openCapsuleId === "2026-06-23T00.0001",
  open_capsule_row_count_matches_overflow: Number(openCapsuleRowCount) === overflowRows,
  open_body_contains_last_row: openCapsuleBody.endsWith(lastCall.history.row),
  history_root_tracks_open_end: historyRoot === openCapsuleEndRoot
};
const passed = Object.values(checks).every(Boolean);
if (!passed) {
  await writeReport({
    schema: "octra-vitals-program-v1-overflow-rehearsal-v0",
    status: "failed",
  ...
  throw new Error(`overflow rehearsal failed: ${JSON.stringify(checks)}`);
}
...
await writeReport({
  ...
  open_capsule: {
    ...
    base_id: openCapsuleBaseId,
    segment: Number(openCapsuleSegment),
