...
Strict-runtime ABI slimming is a successor-era candidate: the state field list and row encodings stay unchanged, but public getters and write methods are removed. Deploy it as a fresh era, not as an in-place code update. Pause the updater, deploy the new programmed Circle, initialize it with the predecessor final root/index, verify era stitching, then point the gateway/updater at the successor only after review.

Normal `npm run program:compile` is verification-only and must match `program-fact-ledger/approved-release.json`. A new in-place candidate is approved only with `npm run program:compile:refresh`, two agreeing compiler RPCs, `VITALS_REFRESH_AML_PINS_ACK=sha256:<candidate-source-hash>`, and a previous compile artifact matching every old approved hash. A successor-era ABI break additionally requires `VITALS_AML_RELEASE_MODE=new_era` and `VITALS_REFRESH_AML_ABI_BREAK_ACK=sha256:<candidate-source-hash>`. Refresh preserves the recovery input as the tracked `program-fact-ledger/previous-approved-compile.json` and a build copy; a clean checkout without a matching current recovery artifact must supply `VITALS_PROGRAM_UPDATE_PREVIOUS_COMPILE_ARTIFACT`. Never refresh pins as a side effect of deployment.
