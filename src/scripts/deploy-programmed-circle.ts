...
const sourcePath = process.env.VITALS_PROGRAMMED_CIRCLE_SOURCE || join(root, "program-circle", "main.aml");
...
async function assertCompileMatchesPinnedArtifacts(source: string, compile: CompileResult, codeB64: string): Promise<void> {
  ...
  const [pinnedVerificationText, pinnedCertificateText, pinnedAbiText, pinnedLowered] = await Promise.all([
    readFile(join(root, "program-circle", "formal_verification.json"), "utf8"),
    readFile(join(root, "program-circle", "formal_certificate.json"), "utf8"),
    readFile(join(root, "program-circle", "abi.json"), "utf8"),
    readFile(join(root, "program-circle", "lowered.oasm"), "utf8")
  ]);
...
const [source, deployOu, updateOu, callOu, deployFee, updateFee, callFee] = await Promise.all([
...
if (!deployEnabled) {
  ...
  const initTx: OctraTransaction = {
    ...
    timestamp: Date.now() / 1000,
    op_type: "circle_call",
    encrypted_data: "initialize_v0",
    message: JSON.stringify([operatorAddress])
  };
  const initSubmission = await submitTx(wallet, initTx);
  const initConfirmation = await requireConfirmed(initSubmission.tx_hash, "programmed Circle initialize_v0");
  ...
  if (requireContractReceipt && (initReceipt?.success !== true || initReceipt?.method !== "initialize_v0" || initReceipt?.contract !== circleId)) {
    throw new Error(`programmed Circle initialize_v0 receipt did not verify: ${JSON.stringify(initReceipt || { error: initReceiptError })}`);
  }

  const [programInfo, manifest, initialized, owner, operator, count] = await Promise.all([
  ...
  if (manifest !== "vitals-circle-state.v0" || !initializedOk || !ownerOk || !operatorOk) {
