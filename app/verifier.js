(function attachOctraVitalsVerifier(root){
  ...
  function verifySnapshotSemantics(input){
    ...
    for(const [index, ref] of effectiveSourceRefs.entries()){
      ...
      if(entry?.kind !== undefined && ref.kind !== undefined) requireEqual(`source ref ${index} kind`, ref.kind, entry.kind);
      if(entry?.method !== undefined && ref.method !== undefined) requireEqual(`source ref ${index} method`, ref.method, entry.method);
      if(entry?.url !== undefined && ref.url !== undefined) requireEqual(`source ref ${index} url`, ref.url, entry.url);
