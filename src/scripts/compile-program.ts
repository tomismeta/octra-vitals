...
const sourcePath = join(root, "program", "main.aml");
const buildDir = join(root, "build", "program");
...
await writeJson(join(root, "program", "abi.json"), abi);
await writeFile(join(root, "program", "lowered.oasm"), `${result.disasm || ""}\n`);
await writeJson(join(root, "program", "formal_verification.json"), result.verification || {});
await writeJson(join(root, "program", "formal_certificate.json"), result.certificate || {});
...
console.log(stableJson({
  ...
  outputs: [
    "program/abi.json",
    "program/lowered.oasm",
    "program/formal_verification.json",
    "program/formal_certificate.json",
    "build/program/compile.json"
  ]
}));
