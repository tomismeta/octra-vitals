...
The lab is deliberately non-canonical and can lag or fail without affecting snapshot collection or the public Vitals page. AML remains the ledger of record, and the SQLite Circle is a derived mirror for discoverability, query ergonomics, and `octra-sqlite` evaluation. In production shape, the canonical Vitals Circle carries AML plus the core web assets; the Vitals Lab Circle carries the lab web assets plus the SQLite database. Lab reads do not require a token; operator repair/backfill syncs do. Vitals does not expose raw JSON-RPC request/response traces from lab queries. Mainnet Lab exposure is opt-in and requires `VITALS_LAB_HISTORY_ALLOW_MAINNET=1`.
