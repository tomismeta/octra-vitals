...
const sourcePath = join(root, "program-fact-ledger-probe", "main.aml");
const buildDir = join(root, "build", "program-fact-ledger-probe");
...
const compileArtifact = {
  schema: "octra-vitals-fact-ledger-probe-compile-v1",
  generated_at: new Date().toISOString().replace(/\.\d{3}Z$/, "Z"),
  source_path: "program-fact-ledger-probe/main.aml",
...
if (verification.verified !== true || verification.safety !== "safe" || verification.errors !== 0 || verification.warnings !== 0) {
  throw new Error(`fact ledger probe formal verification failed: ${JSON.stringify({
...
console.log(stableJson({
  ...
  size: result.size,
  outputs: [
    "build/program-fact-ledger-probe/compile.json"
  ],
