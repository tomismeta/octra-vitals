...
The mirror worker and sync endpoint are chunked and completion-last. Repeated runs backfill missing rows; complete no-op runs exit before any Circle write, and a partial write does not advance the mirror watermark to complete. The lab web assets are published as a separate Lab release to a public Lab Web Circle, not bundled into the canonical Vitals Circle and not hosted by the sealed Lab SQLite Circle.
